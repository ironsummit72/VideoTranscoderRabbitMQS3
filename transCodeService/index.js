...
mongoose.connect(`${process.env.MONGO_URL}/${process.env.DB_NAME}`).then((response)=>{
  console.log(response.connection.host+":"+response.connection.port,"connected to database");
  
  }).catch((error)=>{
  console.error(error);
  })
async function transcoderService()
{
...
channel.consume(queue,async(message)=>{
  ...
  // save the location to Database and key
   try {
    const dbResponse =await videoModel.create({title:data.videoTitle,description:data.videoDescription,videoUrl:element.Location,S3Key:element.Key})
   ...
   console.log(finalVideoResponse,"final video response");
...
return new Promise((resolve,reject)=>{
  ...
  s3.getObject(s3Params).createReadStream().on('end',()=>{
console.log('video downloaded successfully');
resolve(filePath)
  }).on('error',reject).pipe(fileStream)
}
);
}
...
app.listen(port, () => {
  console.log(`transcode service is listening on port ${port}`);
});
